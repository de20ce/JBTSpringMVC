...
public class JBTJdbcController {
	...
	public String insertContact(@ModelAttribute("SpringWeb") VngMem vngmem, ModelMap model) {
		...
		model.addAttribute("dob",vngmem.getDob());
		model.addAttribute("email",vngmem.getEmail());
		model.addAttribute("phone",vngmem.getPhone());
		model.addAttribute("address",vngmem.getAddress());
		model.addAttribute("pincode",vngmem.getPincode());
		model.addAttribute("country",vngmem.getCountry());
		try {
